...
Use the [README development workflow](README.md#development) for routine iteration in the Electron dev window. Local packaging and installation are required for installer/packaging changes, behavior specific to the packaged or installed app, release verification, or an explicit installed-build request. These development rules do not replace the CI or release gates below, and dev-window testing does not establish that a release package works.
