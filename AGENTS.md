...
- `master` is the only long-lived production branch. Create a short-lived `feat/`, `fix/`, `docs/`, or `chore/` branch from the latest `master` for each change and open a pull request into `master`. `dev` and `stage` are retired; "main branch" means `master`.
