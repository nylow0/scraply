<script lang="ts">
  ...
  function missingFields(): Record<string, string> {
    ...
    if (researchMode === "explore-market") {
      if (!domain.trim()) next.domain = "Market or domain is required.";
...
<section class="scope-page">
  <header>
    ...
    <p>Explore a market to discover evidence-backed problems, or skip discovery when you already know the problem.</p>
  </header>
  ...
  <form onsubmit={(event) => { event.preventDefault(); void saveAndStart(); }}>
    <fieldset class="mode-picker">
      ...
      <label class:active={researchMode === "explore-market"}>
        ...
        <span><strong>Explore a market</strong><small>The model searches the market and audience, extracts factors, then combines them into problems.</small></span>
      </label>
    ...
    <div class="primary-fields">
      <label><span>Research name</span><input bind:value={title} aria-invalid={Boolean(errors.title)} placeholder={researchMode === "explore-market" ? "Independent repair shops" : "Repair delay solutions"} />{#if errors.title}<small class="field-error">{errors.title}</small>{/if}</label>
      ...
      {/if}
      <label><span>{researchMode === "explore-market" ? "Market or domain" : "Market or domain (optional)"}</span><input bind:value={domain} aria-invalid={Boolean(errors.domain)} placeholder="Parts sourcing and workshop operations" />{#if errors.domain}<small class="field-error">{errors.domain}</small>{/if}</label>
      <label><span>{researchMode === "explore-market" ? "Audience" : "Audience (optional)"}</span><input bind:value={audience} aria-invalid={Boolean(errors.audience)} placeholder="Owners of 2–10 person repair shops" />{#if errors.audience}<small class="field-error">{errors.audience}</small>{/if}</label>
    </div>

    <details class="optional-fields">
      <summary>Context and boundaries <span>Optional</span></summary>
      <div>
        <label><span>{researchMode === "explore-market" ? "Signals already noticed" : "Context"}</span><small>{researchMode === "explore-market" ? "Steers evidence collection without asserting the answer." : "Useful background for solution generation."}</small><textarea bind:value={observations} rows="4" placeholder="What have you noticed already?"></textarea></label>
