<script lang="ts">
  ...
  import { DEFAULT_RUN_CONFIG, modelRefKey, sameModelRef, type ModelOption, type ModelRef, type RunConfig } from "../../shared/schemas";
  import { modelDisplayName } from "../lib/research-defaults";
  import { untrack } from "svelte";
  import { SvelteSet } from "svelte/reactivity";
  let { problems, rejectedCandidates, modelOptions, initialConfig, busy, onCommit, onExport, onOpenSource }:{ problems:ProblemCandidate[];rejectedCandidates:RejectedProblemCandidate[];modelOptions:ModelOption[];initialConfig:RunConfig|null;workflowVersion?:1|2|undefined;ideaCount?:number|undefined;busy:boolean;onCommit:(ids:string[],userProblem:string|null,model:ModelRef,reasoningEffort:string)=>Promise<void>;onExport:()=>Promise<void>;onOpenSource:(url:string)=>Promise<void> }=$props();
...
<section class="checkpoint problem-review">
  ...
  <footer><p><strong>{selected.size+(userProblem.trim()?1:0)}</strong> problems selected · ~{projected} model calls projected</p><button disabled={busy||!selectedModel||(!selected.size&&!userProblem.trim())} onclick={()=>onCommit([...selected],userProblem.trim()||null,model,reasoningEffort)}>{busy?"Starting…":"Commit selection"}</button></footer>
</section>
<style>
